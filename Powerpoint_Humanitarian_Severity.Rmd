---
title: "Building Humanitarian Severity Index"
...
knitr::opts_chunk$set(echo = FALSE, fig.height = 9, fig.width = 18, dpi = 300, comment = ""  )
...
humseverity <- as.data.frame(data[ ,c("C_101_name")])
 ...
 * Objective is __simplify__ information into a simple indicator that can reduce information overflow.

 * Informs the sectoral and inter-sectoral discussions in order the facilitate __analyis of needs__ between sectors.
 
 * A Severity index allows for __comparability__ between different locations and over time. 
 
 * It is a strong tool to support __advocacy__ at different levels & phases and a requirement from interagency humanitarian needs overview ([HNO](https://www.humanitarianresponse.info/sites/www.humanitarianresponse.info/files/documents/files/Comparison%20Tool%20Guidance%202015%20FINAL-EN.pdf#page=12)) process.


## Data set used for the analysis 

 * comes from __Key Informant Interviews__ affected by Hurricane Dorian, Bahamas  - [data available through HDX here](https://data.humdata.org/dataset/the-bahamas-hurricane-dorian-site-assessment-round-3-november-2019#). 
 
 * Data were collected at the beginning of November 2019 by IOM/DTM in the Bahamas. See [initial report](https://displacement.iom.int/reports/bahamas-%E2%80%94-hurricane-dorian-%E2%80%93-site-assessment-%E2%80%93-round-3-november-2019?close=true) 
 
 * covers Abaco Island which has been severely hit by Hurricane Dorian on 1-3 September 2019.
 
Analysis can be reproduced on the next release of DTM to identify __Severity Evolution over time__.


---

# Methodology: What is badly defined is likely to be badly measured.
 ...
 * How to select and organize indicators? (a consistent Framework)  

 * How to calculate and reshape them? (polarity & normalization)

 * How to assemble them together? (aggregation and weighting).
 
 * What formula to retain? (comparison & sensitivity).



---

## Severity Theoretical Framework


:::::: {.columns}
:::::: {.column}

Composite indicators are generally used to summarize a number of underlying individual indicators or variables. There are basically three levels of indicator groupings:

 *  Individual indicator sets represent a menu of separate indicators or statistics. 
 *  Thematic indicators are individual indicators which are grouped together around a specific area or theme. 
 *  Composite indicators are formed when thematic indicators are compiled into a synthetic index and presented as a single composite measure.

:::
:::::: {.column}
![__Example:__ INFORM Global Risk Index (GRI) Framework](index.png)
:::
::::::::::::::
...
The __polarity__ of a sub-indicator is the sign of the relationship between the indicator and the phenomenon to be measured (_for instance, in a well-being index, "GDP per capita" has 'positive' polarity and "Unemployment rate" has 'negative' polarity_), which means here:

 * Negative (the higher score, the more severe) or 
 
 * Positive (the higher score, the less severe)
 
## Data Normalization 
 
Data Normalization allows for __adjustments of distribution__ (similar range of variation) and scale (common scale) of sub-indicators that may reflect different units of measurement and different ranges of variation. Different possible options:
...
## Calculating sub-indicators values
...
## Correlation & Consistency between Indicators 

Checking correlation between indicators allows to ensure that they actually measure a meaningful composite (if not enough correlated, they are not related, if too correlated, sub-indicators are redundant). This is an essential step to eliminate indicators from the theoretical framework.

Cronbach’s alpha, (or coefficient alpha), also measures reliability / consistency (i.e.  how well a test measures what it should: measure of the stability of test scores). As a rule of thumbs, a score of more than 0.7 indicates an acceptable level of consistency: 
...
Simply using _"equal weight"_ (all indicators account for the same in the final index) and _"arithmetic aggregation"_ (_all indicators are substituable_) is unlikely to depict the complex issue of Humanitarian Severity and is likely to comes with the risk of misrepresenting the reality. 

---

## Benefice of the Doubt

- __Benefit of the Doubt approach (BoD)__, application of Data Envelopment Analysis (DEA), ensures that weights are endogenously determined by the observed performances. Benchmark is not based on theoretical bounds, but it’s a linear combination of the observed best performances.

- __Directional Benefit of the Doubt (D-BoD)__ model enhances non-compensatory property by introducing directional penalties in a standard BoD model in order to consider the preference structure among simple indicators. 

- __Robust Benefit of the Doubt approach (RBoD)__ is an extension of BoD based on the concept of the expected minimum input function.

- __Benefit of the Doubt approach (BoD)__ index with constraints on weights allows for constraints (so that there is none not valued) and with a penalty.

---

## Other algorithms

- __Factor analysis__ groups together simple indicators to estimate a composite indicator that captures as much as possible of the information common to individual indicators.

- __Mean-Min Function__ is an intermediate case between arithmetic mean, according to which no unbalance is penalized, and min function, according to which the penalization is maximum. 

- __Geometric aggregation__ uses the geometric mean to aggregate the single indicators and therefore allows to bypass the full compensability hypothesis using geometric mean. 

- __Mazziotta-Pareto Index__ transforms a set of individual indicators in standardized variables and summarizes them using an arithmetic mean adjusted by a "penalty" coefficient related to the variability of each unit.

- __Wroclaw taxonomy method__ is based on the distance from a theoretical unit characterized by the best performance for all indicators considered.
...
Using each of the algorithm explained above, locations will be ranked differently.


### Sensitivity Analysis

Looking at results, we can identify and retain the most __stable__ aggregation method.
...
# Thematic Dimension: `r this.dimension`
...
## Correlations Network : (Alpha consistency is  `r round(Cronbach.this$total$std.alpha, 2)`)
...
qgraph(cor(this.indic.matrix.norm),
       ...
       legend.cex = 0.8,
       title.cex = 1.7,
       title = paste0("Correlations Network for severity indicators related to",
                      this.dimension ))
```
...
line <- ggplot(this.indic.matrix.norm4.melt, aes(x = Var2,
  ...
  geom_text_repel(
    ...
    size = 4,
    nudge_x = 45 ) +
  labs(title = paste0("Composite Severity Index on ",  this.dimension ),
       subtitle = "Based on various weighting & aggregation approaches") +
  bbc_style() +
...
nn <- ncol(humseverity) + 1
humseverity  <- cbind(humseverity,this.composite)
names(humseverity)[nn] <- paste0(this.dimension,"_",names(this.composite)[1])
...
# Full Composite on Severity: aggregating all themes
...
## Correlation Analysis
this.indic.matrix.norm <- humseverity
row.names(this.indic.matrix.norm) <- this.indic.matrix.norm[ , 1]
this.indic.matrix.norm[ , 1] <- NULL
this.indicators.label <- as.character(names(this.indic.matrix.norm))
this.indic.matrix.norm <- as.matrix(this.indic.matrix.norm)

this.indicators.label2 <- as.data.frame(this.indicators.label)
this.indicators.label2$polarity <- "POS"
this.indicators.label2$dir <- 1
polarity2 <- as.character(this.indicators.label2$polarity)
...
qgraph(cor(this.indic.matrix.norm),
       ...
       legend.cex = 0.8,
       title.cex = 1.7,
       title = "Correlations Network between Humanitarian severity themes")
```
...
CI_Direct_BoD_estimated <-  Compind::ci_bod_dir(this.indic.matrix.norm,
                                       indic_col = (1:ncol(this.indic.matrix.norm)),
                                       dir = as.numeric(this.indicators.label2$dir))
...
humseveritySheet <- xlsx::createSheet(wb, sheetName = "humseverity")
xlsx::addDataFrame(humseverity, humseveritySheet, col.names = TRUE, row.names = TRUE)
xlsx::saveWorkbook(wb, "DTM R3 DB Great-Little Abaco MSLA V5.xlsx")
